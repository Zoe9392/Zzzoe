...
class HarvesterAnt(Ant):
    ...
    food_cost = 2 # Initialization in Problem 1
...
class ThrowerAnt(Ant):
    ...
    food_cost = 3   # Initialization in Problem 1

    min_range = 0 #Ini in Problem 3
    max_range = float('inf') # Ini Problem 3
    ...
    def nearest_bee(self):
        ...
        n_place = self.place # to assign a variable instead of using self.place
        n = 0
        while True:
            if n_place.is_hive == False:  # terminal: .is_hive to reply True or False
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for bee in self.place.bees[:]: 
            ...
            if self.health <= amount and bee.place != None:
                bee.reduce_health(self.damage)
        Ant.reduce_health(self, amount)
        # END Problem 5

  # "reduce the health of each bee: list "
  # ??? remove the FireAnt
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        self.eat_bees(random_bee(self.place.bees))

    
# check chewing, if so ; not chewing == no bees there  
# There are wo helper function here 
# END Problem 7
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        if not (self.ant_contained or isinstance(other, ContainerAnt)):
            return True
        else:
            return False
        # END Problem 8
...
#  to link with the SPECIFIC METHODS of FireAnts 
#  But ???  
#  其余 class 里面的 mehtods 可以被调用
# END Problem 9
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        if cls.number < 1:
            print("scuba construct" , cls.__name__)
        elif gamestate.food_cost < QueenAnt.food_cost:
            print("scuba init" , cls.__name__)
        else:
            return None 
        return cls()

        #    QueenAnt.number += 1
        #    return QueenAnt.number
        #elif gamestate.ant_types(QueenAnt)>=2 :
        #    return None
        #return cls()
    ...
    def action(self, gamestate):
        ...
        if self.number == 0:
            ...
            while n_place != None:
                if n_place.ant != None and n_place.ant.buff == False:
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 12
        Ant.reduce_health(self, amount)
        if self.health <= 0 and self.number == 0: 
            ants_lose()
